...
public class StressorAppRunnerTests
{
    ...
    public async Task Should_ReturnExitCodeZeroAndPrintVersion_When_Version()
    {
        ...
        try
        {
            ...
            Assert.Equal(0, exitCode);
            Assert.Contains("1.0.0-beta", writer.ToString(), StringComparison.Ordinal);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public async Task Should_ApplyBetaFlagsFromConfig_When_ConfigProvidesDurationProgressAndDryRun()
    {
        var configPath = await WriteTempScenarioAsync(
            """
            {
