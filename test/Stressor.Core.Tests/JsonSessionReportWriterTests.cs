...
public class JsonSessionReportWriterTests
{
    ...
    public async Task Should_WriteValidJson_When_ReportProvided()
    {
        ...
        await writer.WriteAsync(report, reportPath, 0, "0.11.0-alpha", TestCancellation.Token);

        var json = await File.ReadAllTextAsync(reportPath, TestCancellation.Token);
        using var document = JsonDocument.Parse(json);
        Assert.Equal(1, document.RootElement.GetProperty("schemaVersion").GetInt32());
        Assert.Equal("0.11.0-alpha", document.RootElement.GetProperty("stressorVersion").GetString());
    ...
    public async Task Should_OverwriteExistingFile_When_PathExists()
    {
        ...
        await writer.WriteAsync(report, reportPath, 1, "0.11.0-alpha", TestCancellation.Token);
    ...
    public async Task Should_ThrowArgumentException_When_PathEmpty()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            writer.WriteAsync(report, "  ", 0, "0.11.0-alpha", TestCancellation.Token));
    }
